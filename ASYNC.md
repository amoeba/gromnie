...
- Use Wasmtime `async_support(true)` and async instantiation/linking support where provided.
...
- Use `wasmtime_wasi::add_to_linker_async` and async host bindings generated by `bindgen!({ ..., async: true })`.
...
| Phase 5: Replace TimerManager with async timers | Completed |
...
- Host-side linking uses `wasmtime_wasi::add_to_linker_async` for WASI and generated async host trait signatures for component imports; the generated component helper remains `add_to_linker`.
