...
mod scene_tests {
    ...
    fn test_character_select_scene_creation() {
        ...
        assert_eq!(scene.entering_world, None);
    }
    ...
    fn test_scene_as_character_select_mut() {
        ...
        if let Some(char_select) = scene.as_character_select_mut() {
            char_select.begin_entering_world(
                999,
                "TestChar".to_string(),
    ...
    fn test_client_error_variants() {
        let _char_error =
            ClientError::CharacterError(asheron_rs::enums::CharacterErrorType::UnspecifiedError);
        let _conn_failed = ClientError::ConnectionFailed("failed".to_string());
        let _patch_failed = ClientError::PatchingFailed("failed".to_string());
        let _login_timeout = ClientError::LoginTimeout;
        let _patch_timeout = ClientError::PatchingTimeout;
...
#[cfg(test)]
mod session_tests {
    use gromnie_client::client::{ClientSession, SessionState};
