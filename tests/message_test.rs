...
fn compare_arrays_with_wildcard(expected: &Vec<u8>, actual: &Vec<u8>) -> bool {
    ...
    let pass = expected.iter().zip(actual.iter()).all(|(&exp, &act)| exp == act || exp == 0xFF);
...
fn test_message_login_request() {
    ...
    assert!(compare_arrays_with_wildcard(&pak_data_root_root, &buffer_root_root.into_inner()));
    ...
    let pak_data_testing_testing = vec![
        ...
        0x00, 0x00, // Padding+ alignment
        0x28, 0x00, 0x00, 0x00, // Length
        0x02, 0x00, 0x00, 0x00, // AuthType
        0x00, 0x00, 0x00, 0x00, // AuthFlags
        0xff, 0xff, 0xff, 0xff, // Sequence
        0x07, 0x00, 0x74, 0x65, 0x73, 0x74, 0x69, 0x6e, 0x67, 0x00, // Account (Len + "testing")
    ...
    assert!(compare_arrays_with_wildcard(&pak_data_testing_testing, &buffer_testing_testing.into_inner()));
    ...
    let pak_data_elevencharsl_elevencharsl = vec![
        ...
        0x00, 0x00, // Iteration
        0x04, 0x00, 0x31, 0x38, 0x30, 0x32, // ClientVersion
        0x00, 0x00, // padding/align ??
        0x31, 0x00, 0x00, 0x00, // Length
        0x02, 0x00, 0x00, 0x00, // AuthType
        0x00, 0x00, 0x00, 0x00, // AuthFlags
        0xff, 0xff, 0xff, 0xff, // Sequence
        0x0c, 0x00, 0x65, 0x6c, 0x65, 0x76, 0x65, 0x6e, 0x63, 0x68, 0x61, 0x72, 0x73, 0x6c, // AccountName
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0d, 0x00, 0x00, 0x00, // ??
        0x0c, 0x65, 0x6c, 0x65, 0x76, 0x65, 0x6e, 0x63, 0x68, 0x61, 0x72, 0x73, 0x6c // Password
    ];
    ...
    assert!(compare_arrays_with_wildcard(&pak_data_elevencharsl_elevencharsl, &buffer_elevencharsl_elevencharsl.into_inner()));
