...
pub fn render_header(frame: &mut Frame, area: Rect, app: &crate::tui::app::App) {
    ...
    let version = env!("VERSION_STRING");
    let title = "Gromnie";
...
pub fn render_tab_bar(frame: &mut Frame, area: Rect, app: &crate::tui::app::App) {
    let default_style = Style::default().bg(Color::Gray).fg(Color::Black);

    let game_style = if matches!(app.current_view, crate::tui::app::AppView::Game) {
        Style::default().bg(Color::White).fg(Color::Black).bold()
    } else {
        default_style
    };

    let debug_style = if matches!(app.current_view, crate::tui::app::AppView::Debug) {
        Style::default().bg(Color::White).fg(Color::Black).bold()
    } else {
        default_style
    };
...
pub fn render_status_bar(frame: &mut Frame, area: Rect, app: &crate::tui::app::App) {
