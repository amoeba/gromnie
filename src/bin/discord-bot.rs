...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let discord_token = std::env::var("DISCORD_TOKEN")
        .map_err(|_| "DISCORD_TOKEN not provided")?;
    ...
    let game_username = std::env::var("GAME_ACCOUNT")
        .map_err(|_| "GAME_ACCOUNT not provided")?;
    
    let game_password = std::env::var("GAME_PASSWORD")
        .map_err(|_| "GAME_PASSWORD not provided")?;
    ...
    let (client_event_tx, _client_event_rx) = 
        tokio::sync::mpsc::unbounded_channel::<GameEvent>();
    let (action_tx_channel, mut action_tx_rx) = 
    ...
    let mut client_handle = tokio::spawn(gromnie::runner::run_client_with_action_channel(
        config,
        move |action_tx| {
            DiscordConsumer::new(action_tx.clone(), http_clone.clone(), channel_id)
        },
        action_tx_channel,
        shutdown_rx,
    ));
